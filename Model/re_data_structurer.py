...
class ReDataStructurer:
    ...
    def structure_data(self, data_path, destination_path):
        """Structures the re data into feature and label data and saves the result into a single file."""
        print("Structuring re data...")
        re_data_file_path = os.path.join(destination_path, "re_data.pickle")
        # Removes the re data file, if it already exists.
        os.remove(re_data_file_path) if os.path.exists(re_data_file_path) else None
        num_re_files = self.count_num_jpg_files(data_path=data_path)
        # Creates the containers for the feature and label data.
        re_feature_data = np.ndarray(shape=(num_re_files, self.FRAME_HEIGHT, self.FRAME_WIDTH, 3), dtype=np.float32)
        re_label_data = np.ndarray(shape=num_re_files, dtype=np.int32)
        re_feature_data_index = 0
        for root, dirs, files in os.walk(data_path):
            for file in files:
                if self.JPG_EXTENSION in file:
                    ...
                    re_feature_data[re_feature_data_index, :, :, :] = image_as_nd_array
                    re_label_data[re_feature_data_index] = label
                    # Updates the position in the containers for the next image.
                    re_feature_data_index += 1
        re_feature_data, re_label_data = self.permute_feature_label_data(feature_data=re_feature_data,
                                                                         label_data=re_label_data)
        re_feature_data = self.scale_feature_data(feature_data=re_feature_data)
        self.create_structured_re_data_file(re_data_file_path=re_data_file_path, feature_data=re_feature_data,
                                            label_data=re_label_data)
    ...
    def create_structured_re_data_file(self, re_data_file_path, feature_data, label_data):
        """Creates a single file containing the structured re data."""
        try:
            f = open(re_data_file_path, "wb")
            structured_re_data = {
                "feature_data": feature_data,
                "label_data": label_data
            }
            pickle.dump(structured_re_data, f, pickle.HIGHEST_PROTOCOL)
            f.close()
        except:
            print("Unable to save the structured re data to {}.".format(re_data_file_path))
            raise
